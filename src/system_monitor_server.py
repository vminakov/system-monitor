...
class SimpleServerProtocol(WampServerProtocol):
   ...
   def onSessionOpen(self):
      ...
      self.registerForPubSub(SimpleServerProtocol.uri)

      model = MemoryChart()

      #reactor.callLater(1, self.pushEvent)
      l = task.LoopingCall(model._refresh)
      l.start(1.0)

   def pushEvent(self):
      self.dispatch("http://example.com/pubsub", "Some data")
